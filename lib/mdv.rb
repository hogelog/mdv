...
class MDV
  ...
  def port_file = File.join(config_dir, "daemon.port")
  def pid_file = File.join(config_dir, "daemon.pid")
  def history_file = File.join(config_dir, "history")
  def authorized_file = File.join(config_dir, "authorized")
  ...
  def decode_path(path)
    path.gsub(/%([0-9A-Fa-f]{2})/) { $1.to_i(16).chr(Encoding::BINARY) }.force_encoding(Encoding::UTF_8)
  end
  ...
  def render_markdown(source)
    Commonmarker.to_html(source, options: {render: {unsafe: false}})
  end
  ...
  def embed_images(html, document)
  ...
  def respond(response, status, body, content_type = "text/html; charset=utf-8")
    response.status = status
    response["Content-Type"] = content_type
    response["Content-Security-Policy"] = "default-src 'none'; style-src 'self'; img-src 'self' data:; script-src 'none'; frame-src 'none'; object-src 'none'"
  ...
  def index
    ...
    body = <<~HTML
      ...
      <footer class="home-footer"><span>mdv #{VERSION}</span><a href="https://github.com/hogelog/mdv">GitHub</a><span>Data: #{escape(config_dir)}</span></footer></div>
    HTML
    page("mdv — Local Markdown Viewer", body)
  end

  def handle(request, response)
    return respond(response, 405, page("mdv error", "<main class=\"error\"><h1>405</h1></main>")) unless request.request_method == "GET"
    path = request.path
    if path == "/"
      respond(response, 200, index)
    elsif path == "/assets/style.css"
      respond(response, 200, STYLE, "text/css; charset=utf-8")
    else
      ...
      if file =~ /\.(png|jpe?g|gif|webp|svg)$/i
        ...
        return respond(response, 403, page("mdv error", "<main class=\"error\"><h1>403</h1><p>image was not opened by mdv</p></main>")) unless allowed
        type = image_content_type(file) || "image/svg+xml"
        respond(response, 200, File.binread(file), type)
      else
        file = markdown_path(file)
        return respond(response, 403, page("mdv error", "<main class=\"error\"><h1>403</h1><p>file was not opened by mdv</p></main>")) unless authorized.include?(file)
        ...
        HTML
        respond(response, 200, page(File.basename(file), body))
      end
    end
  rescue Errno::ENOENT, RuntimeError => error
    respond(response, 404, page("mdv error", "<main class=\"error\"><h1>404</h1><p>#{escape(error.message)}</p></main>"))
  end
